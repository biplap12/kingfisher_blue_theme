...
const Hero2 = () => {
  return (
    <div className="text-[#1e1e1e] min-h-screen w-full">
      {/* Top Info Section */}
      <div className="w-full px-6 py-20 grid grid-cols-2 md:grid-cols-4 text-center">
        {data.map((item, index) => (
          ...
          >
            <p className="text-xs mb-1 uppercase tracking-wider flex justify-center gap-2">
              {item.icon}
              {item.label}
            </p>
  ...
  return (
    <div key={index} className="text-center mb-10">
      <h1 className="text-4xl md:text-5xl tracking-widest text-[#1e2c3a] uppercase mb-6">
        {item.title}
      </h1>
