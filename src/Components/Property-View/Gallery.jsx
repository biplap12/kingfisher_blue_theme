    ...
    return (
        <section className="lightSection min-h-screen mt-20 p-4 md:p-6 font-sans ">
            <div className="max-w-6xl mx-auto bg-white rounded-xl p-4 md:p-8">
                <Link to={'/properties'} className="text-lg flex justify-start items-center cursor-pointer text-blue-600  hover:underline"><MoveLeft size={16}/> Back</Link>
                <h1 className="text-4xl font-semibold heading-font mt-2">{data.title}</h1>
                    ...
                    <div className="grid grid-cols-3 lg:grid-cols-1 gap-3 max-w-11/18 mx-auto my-auto">
                        {data.thumbnails.map((thumb, idx) => (
                            <button
                                ...
                                className={`rounded-xl overflow-hidden border-2 transition-all duration-200 ${mainImage === thumb ? "border-blue-500" : "border-transparent"
                                    }`}
                            >
                                <img
                                    src={thumb}
